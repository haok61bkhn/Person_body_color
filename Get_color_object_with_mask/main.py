...
class Color_object:
    ...
    def get_color(self,image,mask=None,list_mask=[0],num_mask_idx=20,output="hsl"):#image cv2 ,output ="rgb"/"hex","int"
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image = np.array(image).astype(np.uint8)
        list_mask=np.array(list_mask).astype(np.uint8)
        if(mask is None):
            results=fast_colorthief_backend.get_palette(image,self.color_count, self.quality)
        else:
            results=fast_colorthief_backend.get_palette_mask(image, mask,list_mask,num_mask_idx,self.color_count, self.quality)
        if(output=="hex"):
...
if __name__=="__main__":
    ...
    ImageDraw.Draw(bg).polygon(polygon, outline=1, fill=10)
    mask = np.array(bg)

    res=cl_object.get_color(image=img,mask=mask,list_mask=[12,10],output="hsl")
    print(res)
