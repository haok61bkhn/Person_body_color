...
std::vector<color_t> get_palette_mask(py::array_t<uint8_t,  py::array::c_style> image,py::array_t<uint8_t,  py::array::c_style>  mask,py::array_t<uint8_t,  py::array::c_style> mask_idx,int num_mask_id,int color_count, int quality) {
    bool c[num_mask_id];
    ...
    // for (int pixel_index=0; pixel_index < 80; pixel_index += quality) {
        ...
        if(c[data_mask[pixel_index]]){
       
            // std::cout<<int(data[pixel_index * 4 ])<<" "<<int(data[pixel_index * 4 +1])<<" "<<int(data[pixel_index * 4 +2])<<"\n";
            // Not white
            if (data[pixel_index * 3] <= 250 || data[pixel_index * 3 + 1] <= 250 || data[pixel_index * 3 + 2] <= 250) {
...
PYBIND11_MODULE(fast_colorthief_backend, m) {
    ...
    m.def("get_palette_mask", &get_palette_mask, "Return color palette");
};
